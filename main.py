...
st.markdown("Creators: Theo Rostalski, Felix Höner, Justus Rahlf, Loredana Salvi, Moritz Müller, Richard Blanc, Fabian von Widdern")

# Create input fields for calculating the macronutrient distribution
gender = st.selectbox("Geschlecht", gender_list,
                      index=gender_list.index(user["gender"]))
age = st.number_input("Alter", min_value=5, max_value=120,
                      step=1, value=user["age"])
weight = st.number_input("Gewicht (kg)", min_value=1,
                         max_value=500, step=1, value=user["weight"])
height = st.number_input("Größe (cm)", min_value=1,
                         max_value=300, step=1, value=user["height"])
activity_level = st.selectbox(
    "Aktivitätslevel", activity_levels, index=activity_levels.index(user["activity_level"]))
